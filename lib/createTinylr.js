...
module.exports = (opts) => {
  ...
  server.listen(opts.port, opts.host || undefined, () => {
    console.log(`create-tiny-lr: Server is ready`)
    isReady = true
    if (isClosed) {
      console.log(`create-tiny-lr: We are already in closed state, closing server`)
      server.close()
    } else {
      console.log({message: 'LiveReload running on ' + opts.port,})
    }
  })
