...
function build (entry, opts) {
  ...
  mkdirp(outdir, function (err) {
    ...
    compiler.on('change', function (nodeName, edgeName, nodeState) {
      ...
      if (stepName === 'assets:list') writeMultiple('assets', { compress: false })
      if (stepName === 'documents:list') writeMultiple('documents')
      if (stepName === 'service-worker:bundle') {
        var filename = compiler.graph.data.arguments.serviceWorker
    ...
    function writeMultiple (step, opts) {
      opts = opts || {}
      var compress = opts.compress === undefined ? true : opts.compress
      var write = compress ? writeCompressed : writePlain
    ...
    function writeCompressed (filename, buffer, done) {
      async.series([
        ...
        function _brotli (done) {
          try {
            var compressed = brotli.compress(buffer)
          } catch (e) {
            return done(e)
          }
