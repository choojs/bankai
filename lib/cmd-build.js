...
function build (entry, outdir, opts) {
  ...
  mkdirp(outdir, function (err) {
    ...
    compiler.on('change', function (nodeName, edgeName, nodeState) {
      ...
      if (stepName === 'scripts:manifest') writeManifest('scripts')
    ...
    function writeManifest (step) {
      var node = compiler.graph.data[step].manifest
      var filename = path.join(outdir, 'split-require.json')
