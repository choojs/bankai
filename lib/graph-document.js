...
function node (state, createEdge, opts) {
  ...
  serverRender(entry, function (err, render) {
    ...
    function prerender (route, cb) { cb() }

    function renderRoute (route, done) {
      var res
      (opts.onPreRender || prerender)(route, function onPreRenderComplete (err, renderState) {
        if (err) {
          self.emit('ssr', {success: false, error: err})
          return done(err)
        }
        try {
          res = render(route, renderState) // TODO: allow passing in state as second arg
        } catch (err) {
          self.emit('ssr', {success: false, error: err})
          return done(err)
        }
        ...
        var header = [
          viewportTag(),
          renderState && `<script>window.initialState=${JSON.stringify(renderState)}</script>`,
