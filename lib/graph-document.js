...
function node (state, createEdge) {
  ...
  serverRender(entry, function (err, render) {
    ...
    function renderRoute (route, done) {
      ...
      var title = res.title
      var body = res.body
      var baseHref = state.metadata.opts.baseHref
      ...
      var header = [
        viewportTag(),
        scriptTag({ hash: state.scripts.bundle.hash, baseHref: baseHref }),
        ...
        preloadTag(),
        loadFontsTag({ fonts: fonts, baseHref: baseHref }),
        manifestTag({ baseHref: baseHref }),
      ...
      d.transform(addToHead, styleTag({ hash: state.styles.bundle.hash, baseHref: baseHref }))
...
function scriptTag (opts) {
  ...
  var link = `${opts.baseHref || ''}/${hex}/bundle.js`
...
function dynamicScriptsTag (opts) {
  return opts.bundleNames.map(function (name) {
    ...
    var link = `${opts.baseHref || ''}/${hex}/${name}.js`
...
function styleTag (opts) {
  var hex = opts.hash.toString('hex').slice(0, 16)
  var link = `${opts.baseHref || ''}/${hex}/bundle.css`
...
function loadFontsTag (opts) {
  return opts.fonts.reduce(function (html, font) {
    if (!path.isAbsolute(font)) font = (opts.baseHref || '') + '/' + font
...
function manifestTag (opts) {
  return `
    <link rel="manifest" href="${opts.baseHref || ''}/manifest.json">
  `.replace(/\n +/g, '')
}
