...
var scrollbox = require('../../../goto-bus-stop/ansi-scrollbox')
...
function createUi (compiler, state) {
  ...
  compiler.ssr.console.on('data', function (chunk) {
    state.log.setContent(state.log.content + chunk.toString())
    render()
  })
  ...
  function renderMain (state) {
    process.stdout.write(diff.update(mainView(state)))
  }
  function renderLog (state) {
    process.stdout.write(diff.update(logView(state)))
  }
  ...
  function toMainView () {
    ...
    currentRenderer = renderMain
  }

  // Switch to the log view.
  function toLogView () {
    process.stdout.write(ansi.cursor.position(0, 0))
    currentRenderer = renderLog
  }
