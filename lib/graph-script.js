  ...
  function bundleScripts () {
    ...
    b.bundle(function (err, bundle) {
      ...
      exorcise(bundle, mapName, function (err, bundle, map) {
        ...
        // prevent reload browser, when only css file modified.
        if (state.scripts.bundle) {
          var re = /;\(\(null \|\| true\) && "([\w-]+)"/g
          var oldBuf = state.scripts.bundle.buffer.toString()
          var newBuf = bundle.toString()
          if (oldBuf.replace(re, '') === newBuf.replace(re, '')) {
            return self.emit('progress', 'scripts', 100)
          }
        }
