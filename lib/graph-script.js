...
function node (state, createEdge) {
  ...
  // Add this plugin before other transforms, so it can deal with dynamic `import()` syntax.
  ...
  // TODO replace with a new tinyify pipeline API
  b.on('import.pipeline', function (pipeline) {
    pipeline.get('pack').splice(0, 1,
      require('browser-pack')({ raw: true }))
    pipeline.get('pack').push(require('minify-stream')({
