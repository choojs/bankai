...
function node (state, createEdge) {
  ...
  function bundleManifest (manifest, cb) {
    Object.keys(manifest.bundles).forEach(function (name) {
      manifest.bundles[name] = manifest.bundles[name].map(function (filename) {
        return path.relative(state.metadata.dirname, filename)
      })
    })
    createEdge('manifest', Buffer.from(JSON.stringify(manifest, null, 2)), {
      mime: 'application/json'
    })
    cb()
  }
