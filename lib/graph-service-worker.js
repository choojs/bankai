    ...
    find(basedir, filenames, function (err, filename) {
      ...
      function bundleScripts () {
        b.bundle(function (err, bundle) {
          if (err) return self.emit('error', explain(err, 'bankai.service-worker: an error occured in browserify during the .bundle() step'))
...
function fileEnv (state) {
  var script = [`/scripts/${state.scripts.bundle.hash}/bundle.js`]
  var style = [`/styles/${state.style.bundle.hash}/bundle.css`]
